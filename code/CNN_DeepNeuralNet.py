...
def RandomForestClassifier_func(fcTrainData, fcTrainLabel , fcTestData, fcTestLabel,NumberOfEstimators):
    ...
    f = open(Directory+'model/TestPredLabels.txt','w')  # w : writing mode  /  r : reading mode  /  a  :  appending mode
    f.write('{}'.format(pred))
    f.close()

    f = open(Directory+'model/TestLabels.txt','w')  # w : writing mode  /  r : reading mode  /  a  :  appending mode
    f.write('{}'.format(fcTestLabel2))
    f.close()

    return Accuracy , pred , fcTestLabel2
...
if state == 'FullModel':
    ...
    Accuracy , TestPred , TestLabel = RandomForestClassifier_func(fcTrainData, fcTrainLabel, fcTestData, fcTestLabel , RandomForestNumEstimators)

a = roc_curve(TestPred , TestLabel)
b = auc(a[0],a[1])
print(b)
